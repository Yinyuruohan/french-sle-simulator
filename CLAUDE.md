...
- **RC question bank:** `tools/reading_question_bank.py` mirrors `tools/question_bank.py` but for Reading Comprehension. Schema differs (1 question per context, `stem_family` instead of `grammar_topics`, `has_signature` flag, `topic` label — older DBs migrated in place via `ALTER TABLE`). Reviewer is rule-based (`tools/review_reading_exam.py`) — no LLM call. Same lifecycle (`reviewed` → `battle_tested`; `warned` permanent). Assembly ordering differs from WE: `times_served` ASC first (fresh material before repeats), then `reviewed` > `battle_tested` > `warned` > user-flagged, then `RANDOM()`; the selected contexts are shuffled before numbering and there is no stem-family balancing. **Topic diversity:** every generated passage carries a short French `topic` label; the generation prompt forbids two passages on the same subject within one exam, and both the fresh path and `prefill_bank` pass `avoid_topics=get_recent_topics()` (last 20 distinct cached topics) so new exams avoid subjects already in the bank. DB file: `reading_question_bank.db` (gitignored).
