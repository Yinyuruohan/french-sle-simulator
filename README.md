...
Built on the **WAT framework** (Workflows, Agents, Tools) using DeepSeek as the AI engine and Streamlit as the web UI.
...
- **Automated quality review** — adversarial QA agent validates every exam before you see it:
...
- **Adaptive question generation** — focuses on your weak grammar areas over time
...
├── tools/
│   ├── generate_exam.py             # DeepSeek API: generate exam questions + option shuffling
│   ├── evaluate_exam.py             # DeepSeek API: grade + explain answers
│   └── review_exam.py               # DeepSeek API: adversarial QA review of exams and feedback
├── workflows/
...
1. **Start an exam** — choose the number of questions (5-40)
2. **Generation** — DeepSeek generates questions; options are randomized; an adversarial QA review validates quality and regenerates any flagged contexts
...
6. **Improve over time** — the system tracks your errors and generates more questions on your weak areas
...
- ~75% fill-in-the-blank, ~25% error identification (matching the real exam ratio)
...
2. **AI review** — adversarial QA agent at temperature 0.1 checks answer keys, distractors, passage grammar
...
- **AI Engine:** DeepSeek via OpenAI-compatible SDK
- **Web UI:** Streamlit
